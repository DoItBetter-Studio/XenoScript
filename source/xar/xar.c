...
const char *xar_result_str(XarResult r) {
    switch (r) {
        case XAR_OK:           return "ok";
        case XAR_ERR_IO:       return "I/O error";
        case XAR_ERR_BAD_MAGIC:return "bad magic (not a .xar file)";
        case XAR_ERR_CORRUPT:  return "corrupt or truncated data";
        case XAR_ERR_OOM:      return "out of memory";
        case XAR_ERR_TOO_MANY: return "too many chunks or exports";
...
typedef struct { uint8_t *data; size_t size, cap; } WBuf;

static bool wb_init(WBuf *w) {
    w->data = malloc(512); w->size = 0; w->cap = 512;
    return w->data != NULL;
}
static void wb_free(WBuf *w) { free(w->data); }
static bool wb_ensure(WBuf *w, size_t n) {
    if (w->size + n <= w->cap) return true;
    size_t nc = w->cap * 2;
    while (nc < w->size + n) nc *= 2;
    uint8_t *p = realloc(w->data, nc);
    if (!p) return false;
    w->data = p; w->cap = nc; return true;
}
static bool wb_bytes(WBuf *w, const void *d, size_t n) {
    if (!wb_ensure(w, n)) return false;
    memcpy(w->data + w->size, d, n);
    w->size += n; return true;
}
static bool wb_u32(WBuf *w, uint32_t v) {
    uint8_t b[4]; write_u32_be(b, v); return wb_bytes(w, b, 4);
}
...
#define J(lit) do { \
    ...
    if (pos + _l < cap) { memcpy(out + pos, (lit), _l); pos += _l; } \
} while(0)
...
char *xar_manifest_to_json(const XarManifest *m) {
    ...
    J("{");
    J("\"name\":"); json_str(out, &pos, cap, m->name); J(",");
    J("\"version\":"); json_str(out, &pos, cap, m->version); J(",");
    J("\"author\":"); json_str(out, &pos, cap, m->author); J(",");
    J("\"description\":"); json_str(out, &pos, cap, m->description); J(",");
...
bool xar_manifest_from_json(XarManifest *m, const char *json, size_t len) {
    ...
    while (*p) {
        ...
        if (strcmp(key, "name") == 0) {
            ...
            if (*p == '"') {
                char tmp[512]; p = json_parse_str(p, tmp, sizeof(tmp));
            } else {
                while (*p && *p != ',' && *p != '}') p++;
            }
        }
...
static XarResult serialise(WBuf *w,
    ...
    /* Magic */
    if (!wb_bytes(w, XAR_MAGIC, XAR_MAGIC_LEN)) return XAR_ERR_OOM;

    /* Manifest JSON */
    char *json = xar_manifest_to_json(manifest);
    if (!json) return XAR_ERR_OOM;
    uint32_t jlen = (uint32_t)strlen(json);
    bool ok = wb_u32(w, jlen) && wb_bytes(w, json, jlen);
    free(json);
    if (!ok) return XAR_ERR_OOM;

    /* Chunks */
    if (!wb_u32(w, (uint32_t)n_chunks)) return XAR_ERR_OOM;
    for (int i = 0; i < n_chunks; i++) {
        const XarChunk *c = &chunks[i];
        if (!wb_str(w, c->name))                       return XAR_ERR_OOM;
        if (!wb_u32(w, (uint32_t)c->size))             return XAR_ERR_OOM;
        if (!wb_bytes(w, c->data, c->size))            return XAR_ERR_OOM;
    }
    return XAR_OK;
}
...
XarResult xar_write_mem(uint8_t **out, size_t *out_size,
                         ...
                         const XarChunk *chunks, int n_chunks) {
    WBuf w;
    if (!wb_init(&w)) return XAR_ERR_OOM;
    XarResult r = serialise(&w, manifest, chunks, n_chunks);
    if (r != XAR_OK) { wb_free(&w); return r; }
    *out      = w.data;
    *out_size = w.size;
    return XAR_OK;
}
...
XarResult xar_write(const char *path,
    ...
    if (r != XAR_OK) return r;

    FILE *f = fopen(path, "wb");
    if (!f) { free(buf); return XAR_ERR_IO; }
...
typedef struct { const uint8_t *p; const uint8_t *end; } RBuf;
...
XarResult xar_read_mem(XarArchive *ar, const uint8_t *buf, size_t size) {
    ...
    /* Magic */
    char magic[4];
    if (!rb_bytes(&r, magic, 4)) return XAR_ERR_CORRUPT;
    if (memcmp(magic, XAR_MAGIC, 4) != 0) return XAR_ERR_BAD_MAGIC;

    /* Manifest JSON */
    uint32_t jlen;
    if (!rb_u32(&r, &jlen)) return XAR_ERR_CORRUPT;
    if ((size_t)(r.end - r.p) < jlen) return XAR_ERR_CORRUPT;
    ...
    /* Chunks */
    uint32_t n_chunks;
    if (!rb_u32(&r, &n_chunks)) return XAR_ERR_CORRUPT;
    if (n_chunks > XAR_MAX_CHUNKS) return XAR_ERR_TOO_MANY;

    for (uint32_t i = 0; i < n_chunks; i++) {
        XarChunk *c = &ar->chunks[i];

        uint32_t name_len;
        if (!rb_u32(&r, &name_len)) return XAR_ERR_CORRUPT;
        if (name_len >= XAR_MAX_NAME) return XAR_ERR_CORRUPT;
        if (!rb_bytes(&r, c->name, name_len)) return XAR_ERR_CORRUPT;
        c->name[name_len] = '\0';

        uint32_t data_len;
        if (!rb_u32(&r, &data_len)) return XAR_ERR_CORRUPT;
        if ((size_t)(r.end - r.p) < data_len) return XAR_ERR_CORRUPT;
...
XarResult xar_read(XarArchive *ar, const char *path) {
    ...
    if (!buf) { fclose(f); return XAR_ERR_OOM; }

    size_t nr = fread(buf, 1, sz, f);
    fclose(f);
    if ((long)nr != sz) { free(buf); return XAR_ERR_IO; }
